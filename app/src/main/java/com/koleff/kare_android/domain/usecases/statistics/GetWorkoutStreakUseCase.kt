...
class GetWorkoutStreakUseCase(private val generalStatisticsRepository: GeneralStatisticsRepository) {
    suspend operator fun invoke(): Flow<WorkoutStreakState> =
        generalStatisticsRepository.getWorkoutStreak().map { apiResult ->
            when (apiResult) {
                ...
                is ResultWrapper.Loading -> {
                    WorkoutStreakState(isLoading = true)
                }

                is ResultWrapper.Success -> {
                    WorkoutStreakState(
                        isSuccessful = true,
                        streak = apiResult.data.streak
                    )
                }
            }
        }
