...
class SubmitExerciseUseCase(private val workoutRepository: WorkoutRepository) {

    suspend operator fun invoke(workoutId: Int, exercise: ExerciseDto): Flow<WorkoutDetailsState> =
        workoutRepository.submitExercise(workoutId, exercise).map { apiResult ->
            when (apiResult) {
                ...
                is ResultWrapper.Success -> {
                    Log.d("SubmitExerciseUseCase", "Workout with id $workoutId updated\n. Exercise submitted: $exercise")
