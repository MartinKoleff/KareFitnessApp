...
class GetDistinctWorkoutsCompletedUseCase(private val generalStatisticsRepository: GeneralStatisticsRepository) {
    suspend operator fun invoke(): Flow<TotalTimesCompletedState> =
        generalStatisticsRepository.getDistinctWorkoutsCompleted().map { apiResult ->
            when (apiResult) {
