...
class GetMostTrainedExerciseUseCase(private val generalStatisticsRepository: GeneralStatisticsRepository) {
    suspend operator fun invoke(): Flow<ExerciseState> =
        generalStatisticsRepository.getMostTrainedExercise().map { apiResult ->
            when (apiResult) {
