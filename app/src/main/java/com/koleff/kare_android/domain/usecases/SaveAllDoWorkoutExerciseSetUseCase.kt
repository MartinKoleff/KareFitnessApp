...
class SaveAllDoWorkoutExerciseSetUseCase(private val repository: DoWorkoutPerformanceMetricsRepository) {
    suspend operator fun invoke(exerciseSets: List<DoWorkoutExerciseSetDto>): Flow<BaseState> =
        repository.saveAllDoWorkoutExerciseSets(exerciseSets).map { apiResult ->
            when (apiResult) {
                ...
                is ResultWrapper.Success -> {
                    Log.d(
                        ...
                        "Do workout exercise set metrics saved! Exercise sets: $exerciseSets"
                    )
